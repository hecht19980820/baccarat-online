...
def get_mixed_ai_history(platform, table):
    table_history = get_ai_history(
        "WHERE platform=? AND table_no=?",
        (platform, table),
        500
    )

    platform_history = get_ai_history(
        "WHERE platform=? AND table_no<>?",
        (platform, table),
        500
    )

    global_history = get_ai_history(
...
def calc_pattern_routes(bp):
    ...
    return {
        ...
        "routePower": abs(cockroach_bias) + abs(hollow_bias)
    }
...
def road_stats(data):
    ...
    banker_score += (banker_rate - 50) * 0.55
    player_score += (player_rate - 50) * 0.55

    banker_score += (recent_b - recent_p) * 2.6
    player_score += (recent_p - recent_b) * 2.6
    ...
    banker_point_6 = len([
        ...
        if x.get("result") == "B" and x.get("bankerPoint") == 6
    ])
    pair_count = len([
        x for x in recent_cards
        if x.get("playerPair") or x.get("bankerPair")
    ])
    ...
    if lucky6_score >= 15:
        alerts.append("Lucky 6 機率偏高")
    if banker_score >= 65:
        alerts.append("莊方勝率偏高")
    if player_score >= 65:
        alerts.append("閒方勝率偏高")
    if not alerts:
...
def weighted_stats_for_table(platform, table, fallback_screen_data=None):
    ...
    def wavg(key):
        return round(
            table_stats.get(key, 0) * weights["table"] +
            platform_stats.get(key, 0) * weights["platform"] +
            global_stats.get(key, 0) * weights["global"],
            1
        )
    ...
    if lucky6_score >= 15:
        alerts.append("Lucky 6 機率偏高")
    if not alerts:
        alerts.append("共享AI混合模型分析中")

    return {
        "bankerRate": wavg("bankerRate"),
        "playerRate": wavg("playerRate"),
        "tieRate": wavg("tieRate"),
        "bankerScore": banker_score,
        "playerScore": player_score,
        "tieScore": tie_score,
        "lucky6Score": lucky6_score,
        ...
        "totalAnalysis": (
            table_stats.get("totalAnalysis", 0) +
            platform_stats.get("totalAnalysis", 0) +
            global_stats.get("totalAnalysis", 0)
        ),
        "streakResult": table_stats.get("streakResult"),
        "streakCount": table_stats.get("streakCount", 0),
        "jumpRate": table_stats.get("jumpRate", 0),
        "routePower": wavg("routePower"),
        "modelMix": "本桌70%＋同平台20%＋全站10%",
    ...
    for platform, tables in [("DG", DG_TABLES), ("MT", MT_TABLES)]:
        for table in tables:
            ...
            result.append({
                ...
                "ai": ai_stats["suggest"],
                "bankerScore": ai_stats["bankerScore"],
                "playerScore": ai_stats["playerScore"],
                "tieScore": ai_stats["tieScore"],
                "lucky6Score": ai_stats["lucky6Score"],
                "status": status,
                "note": note,
...
def api_admin_core_stats():
    ...
    """).fetchone()["c"]
    maintenance_tables = conn.execute("""
    SELECT COUNT(*) c
    FROM table_status
...
def api_manual():
    ...
    (record_id, platform, table_no, result, cards,
     player_point, banker_point, player_pair, banker_pair, lucky6, tie,
     source, username, created_at)
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    """, (
        record_id, platform, table, result, "",
        0, 0, 0, 0, 0, 1 if result == "T" else 0,
        "manual", session.get("member", ""), now()
    ))
...
def api_cards():
    ...
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    """, (
        record_id, platform, table, calc["result"], json.dumps(cards),
        calc["playerPoint"], calc["bankerPoint"],
        1 if calc["playerPair"] else 0,
        1 if calc["bankerPair"] else 0,
        1 if calc["lucky6"] else 0,
...
def api_clear():
    ...
    conn = db()
    # 清桌只清畫面資料，不刪 shared_ai_stats，AI 學習資料永久保留。
    conn.execute("""
    DELETE FROM records
    WHERE platform=? AND table_no=?
    """, (platform, table))
